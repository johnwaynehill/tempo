...
## 2026-05-22
- Fix design system page rendering blank in production and switch its URL to the cleaner `/design-system`. Was at `/design-system.html` and `serve-handler`'s default `cleanUrls: true` was 301-redirecting to `/design-system`; the `-s` SPA catch-all then served `index.html`, which has no React Router route for `/design-system` so the page rendered blank. Fix has three parts: (1) move `public/design-system.html` → `public/design-system/index.html`; (2) drop `-s` from the Dockerfile and put the SPA catch-all in `public/serve.json` instead — `-s` overwrites the entire `rewrites` array, so it can't coexist with rewrites for specific paths; (3) the catch-all source is `!**/*.html` (minimatch negation) so it doesn't recursively re-rewrite `/design-system/index.html` after the specific rewrite fires (`serve-handler`'s `applyRewrites` is recursive, removing each matched rule and looping the result through the remaining ones). Settings link now `/design-system` to match the rest of the app's URLs. Legacy `/design-system.html` bookmarks still work via `cleanUrls`' built-in `.html` → clean-URL redirect.
